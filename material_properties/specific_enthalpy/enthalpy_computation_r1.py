...
MOLAR_WEIGHTS = {
    ...
    'PA': 231.03588, 'NP': 237.0, 'PU': 244.0,
...
class EnthalpyAnalyzer:
    def __init__(self):
        ...
        # FIXED: Use os.path.join to get the correct database path
...
def normalize_composition(composition_dict, mode='mole'):
    """
    Normalize composition dictionary to ensure sum = 1.0
    For n-component system, if n-1 component fractions are provided,
    the nth component is calculated as 1 - sum(others)
    
    Args:
        composition_dict: Dictionary of element: fraction
        mode: 'mole' or 'weight' fraction normalization
    ...
    # If some fractions are 0 or missing, we'll normalize
    if total > 0:
        # Normalize all components
        normalized = {k: v/total for k, v in composition_dict.items()}
    else:
        # All zeros - equal distribution
        n = len(composition_dict)
        normalized = {k: 1.0/n for k in composition_dict.keys()}
...
def create_enhanced_visualization(df, composition, material_name="Alloy"):
    ...
    cmap = plt.cm.viridis
    color_idx = hash(str(composition)) % 256 / 256.0
...
def main():
    ...
    with tab1:
        ...
        with col_set1:
            ...
            # Composition input with automatic normalization for n-component system
            st.markdown("**Composition (mole fractions):**")
            composition = {}
            
            # For n-component system: if user provides n-1 fractions, nth is calculated automatically
            st.info(f"**System**: {len(selected_elements)}-component alloy. Enter fractions for {len(selected_elements)-1} components, last will be auto-calculated.")
            
            # Create input fields for all but the last element
            for i, element in enumerate(selected_elements[:-1]):
                fraction = st.number_input(
                    ...
                    step=0.01,
                    key=f"comp_{element}",
                    help=f"Fraction of {element} (0.0 to 1.0)"
                )
                composition[element] = fraction
            
            # Calculate the last element's fraction
            entered_sum = sum(composition.values())
            if entered_sum < 1.0:
                last_element = selected_elements[-1]
                last_fraction = 1.0 - entered_sum
                
                st.markdown(f"""
                <div class="success-box">
                <strong>Auto-calculated:</strong> X({last_element}) = 1.0 - Σ(others) = {last_fraction:.4f}
                </div>
                """, unsafe_allow_html=True)
                
                composition[last_element] = last_fraction
            else:
                st.warning(f"Sum of entered fractions = {entered_sum:.4f}. Must be less than 1.0")
                st.stop()
            
            # Display normalized composition
            normalized_comp = normalize_composition(composition, mode='mole')
            st.markdown(f"**Normalized composition**: {', '.join([f'X({k})={v:.4f}' for k, v in normalized_comp.items()])}")
            composition = normalized_comp  # Use normalized composition
        ...
        with col_set2:
            ...
            with st.expander("⚙️ Advanced Options"):
                calc_mode = st.selectbox(
                    "Calculation mode:",
                    ["Equilibrium", "Single phase"],
                    help="Equilibrium: Calculate stable phase equilibrium. Single phase: Calculate properties for a specific phase."
                )
                
                if calc_mode == "Single phase":
                    single_phase = st.selectbox("Select phase:", available_phases)
                    selected_phases = [single_phase]
        ...
        if st.button("🚀 Compute Enthalpy", type="primary", use_container_width=True):
            with st.spinner("🔄 Performing equilibrium calculation... This may take a moment"):
                try:
                    ...
                    # For n-component system, we need to specify compositions for n-1 components
                    # The nth component is automatically calculated by pycalphad
                    elements_with_composition = list(composition.keys())
                    
                    # Add composition for all but the last element
                    for i, element in enumerate(elements_with_composition[:-1]):
                        if composition[element] > 0:
                            conditions[v.X(element)] = composition[element]
                    
                    # Note: The last element's composition is automatically determined
                    # by pycalphad to satisfy the sum constraint
                    ...
                    material_name = "-".join([f"{e}{composition[e]:.2f}" for e in selected_elements[:3]])
                    ...
                    with col_dl2:
                        ...
                        metadata = {
                            ...
                            'Components': ', '.join(selected_elements),
                            'Composition': ', '.join([f'{e}={v:.4f}' for e, v in composition.items()])
                        }
    ...
    with tab4:
        ...
        with col_h1:
            ...
            **3. Multi-Material Comparison**
            ...
            ### N-Component System Handling
            - For n-component systems, enter fractions for n-1 components
            - The nth component is automatically calculated as: 1 - Σ(others)
        ...
        with col_h2:
            ...
            if tdb_files:
                st.write(f"**Found {len(tdb_files)} TDB files in `{analyzer.database_dir}`:**")
                ...
                st.info(f"No TDB files in database directory `{analyzer.database_dir}`. Upload files in Tab 1 to populate.")
