...
• Points: {len(df)}"""
    ...
    def create_comparison_visualization(self, analyzer, selected_indices, customizations=None):
    ...
    def create_interactive_plotly_visualization(self, df, composition, material_name, fitted_params=None):
    ...
    def create_curve_fitting_visualization(self, T_data, H_data, T_fit, H_fit, fit_params, residuals,
        ...
        # Coefficient summary with enhanced formatting (including specific)
        ax4 = fig.add_subplot(gs[2, 2])
        ax4.axis('off')
        # Format coefficients with proper alignment and spacing
        coeff_text = (
            f"🔧 **Molar Parameters (J/mol, K):**\n"
    ...
    def create_phase_diagram_visualization(self, analyzer, selected_indices, customizations=None):
...
def main():
        ...
        if fit_button:
            with st.spinner("Fitting curve to data..."):
                try:
                    ...
                    # Compute specific enthalpy coefficients
                    molar_weight_kg = molar_weight / 1000.0
                    A1_spec = A1_fit / molar_weight_kg
                    A2_spec = A2_fit / molar_weight_kg
                    DeltaHf_spec = DeltaHf_fit / molar_weight_kg
                    ...
                    # Display specific enthalpy coefficients
                    st.markdown("#### 🧮 Specific Enthalpy Coefficients (J/kg, K)")
                    coeff_spec_df = pd.DataFrame({
                        'Parameter': ['A₁ (J/(kg·K))', 'A₂ (J/(kg·K))', 'Tₘ (K)', 'ΔHf (J/kg)', 'k (1/K)', 'H₂₉₈ (J/kg)'],
                        'Value': [f"{A1_spec:.4f}", f"{A2_spec:.4f}", f"{Tm_fit:.2f}",
                                 f"{DeltaHf_spec:,.0f}", f"{k_fit:.6f}", f"{H298_spec:,.0f}"]
                    })
                    st.dataframe(coeff_spec_df, width='stretch', hide_index=True)
    ...
    with tab5:
        ...
        with cust_tab1:
            ...
            with col_cust1:
                # Curve thickness (float)
                analyzer.plot_customizations['curve_thickness'] = st.slider(
                    "Curve Thickness", 0.5, 5.0,
                    value=float(analyzer.plot_customizations.get('curve_thickness', 2.5)),
                    step=0.1, key="tab5_curve_thickness"
                )
                # Box thickness (float)
                analyzer.plot_customizations['box_thickness'] = st.slider(
                    "Box/Spine Thickness", 0.5, 5.0,
                    value=float(analyzer.plot_customizations.get('box_thickness', 1.0)),
                    step=0.1, key="tab5_box_thickness"
                )
                # Marker size (integer)
                analyzer.plot_customizations['marker_size'] = st.slider(
                    "Marker Size", 2, 15,
                    value=int(analyzer.plot_customizations.get('marker_size', 6)),
                    step=1, key="tab5_marker_size"
                )
                # Grid alpha (float)
                analyzer.plot_customizations['grid_alpha'] = st.slider(
                    "Grid Transparency", 0.0, 1.0,
                    value=float(analyzer.plot_customizations.get('grid_alpha', 0.3)),
                    step=0.05, key="tab5_grid_alpha"
                )
            with col_cust2:
                # Font size (integer)
                analyzer.plot_customizations['font_size'] = st.slider(
                    "Font Size", 8, 20,
                    value=int(analyzer.plot_customizations.get('font_size', 12)),
                    step=1, key="tab5_font_size"
                )
                # Title font size (integer)
                analyzer.plot_customizations['title_font_size'] = st.slider(
                    "Title Font Size", 10, 24,
                    value=int(analyzer.plot_customizations.get('title_font_size', 14)),
                    step=1, key="tab5_title_font_size"
                )
                # Legend font size (integer)
                analyzer.plot_customizations['legend_font_size'] = st.slider(
                    "Legend Font Size", 8, 16,
                    value=int(analyzer.plot_customizations.get('legend_font_size', 10)),
                    step=1, key="tab5_legend_font_size"
                )
                ...
                analyzer.plot_customizations['legend_location'] = st.selectbox(
                    ...
                    index=['best', 'upper right', 'upper left', 'lower left', 'lower right',
                          'right', 'center left', 'center right', 'lower center',
                          'upper center', 'center'].index(
                        analyzer.plot_customizations.get('legend_location', 'best')
                    ),
        ...
        with cust_tab2:
            ...
            with col_style1:
                analyzer.plot_customizations['colormap'] = st.selectbox(
                    "Select Colormap", COLORMAPS,
                    index=COLORMAPS.index(analyzer.plot_customizations.get('colormap', 'viridis')),
                    key="tab5_colormap"
                )
                # Preview colormap
                ...
                analyzer.plot_customizations['plot_style'] = st.selectbox(
                    "Plot Style", ['default'] + available_styles,
                    index=0, key="tab5_plot_style"
                )
            with col_style2:
                # Tick size (integer)
                analyzer.plot_customizations['tick_size'] = st.slider(
                    "Tick Label Size", 8, 16,
                    value=int(analyzer.plot_customizations.get('tick_size', 11)),
                    step=1, key="tab5_tick_size"
                )
                # Axis label size (integer)
                analyzer.plot_customizations['label_fontsize'] = st.slider(
                    "Axis Label Size", 8, 16,
                    value=int(analyzer.plot_customizations.get('label_fontsize', 12)),
                    step=1, key="tab5_label_fontsize"
                )
                # Annotation font size (integer)
                analyzer.plot_customizations['annotation_fontsize'] = st.slider(
                    "Annotation Font Size", 6, 14,
                    value=int(analyzer.plot_customizations.get('annotation_fontsize', 9)),
                    step=1, key="tab5_annotation_fontsize"
                )
        ...
        with cust_tab3:
            ...
            with col_layout1:
                ...
                analyzer.plot_customizations['figure_width'] = st.slider(
                    "Figure Width (inches)", 8.0, 20.0,
                    value=float(analyzer.plot_customizations.get('figure_width', 14.0)),
                    step=0.5, key="tab5_figure_width"
                )
                # Figure height (float)
                analyzer.plot_customizations['figure_height'] = st.slider(
                    "Figure Height (inches)", 6.0, 16.0,
                    value=float(analyzer.plot_customizations.get('figure_height', 10.0)),
                    step=0.5, key="tab5_figure_height"
                )
            with col_layout2:
                # DPI (integer)
                analyzer.plot_customizations['dpi'] = st.slider(
                    "Figure DPI", 72, 300,
                    value=int(analyzer.plot_customizations.get('dpi', 150)),
                    step=10, key="tab5_dpi"
                )
                export_format = st.selectbox(
                    "Default Export Format", ['PNG', 'PDF', 'SVG', 'EPS'],
                    index=0, key="tab5_export_format"
                )
        ...
        with col_save1:
            if st.button("💾 Save Customizations", width='stretch', key="tab5_save_cust"):
                st.success("✅ Customizations saved!")
                st.rerun()
        with col_save2:
            if st.button("🔄 Reset to Defaults", width='stretch', key="tab5_reset_cust"):
                analyzer.plot_customizations = {
                    'curve_thickness': 2.5,
                    'box_thickness': 1.0,
                    'font_size': 12,
                ...
                st.rerun()
        with col_save3:
            if st.button("🎨 Apply to All Plots", width='stretch', key="tab5_apply_cust"):
    ...
    with tab6:
        ...
        with col_h1:
            ...
            with st.expander("📊 Fitting Equation", expanded=False):
                ...
                **Parameters:**
                - **A₁**: Sensible heat coefficient (solid phase) [J/(mol·K)]
                - **A₂**: Additional coefficient (liquid phase) [J/(mol·K)]
                - **Tₘ**: Melting/transition temperature [K]
                - **ΔHf**: Heat of fusion/latent heat [J/mol]
                - **k**: Sigmoid steepness parameter [1/K]
                - **H₂₉₈**: Reference enthalpy at 298 K [J/mol]

                **Specific Enthalpy (J/kg):**
                - Automatically calculated from molar enthalpy
                - Uses alloy molar weight: `H_specific = H_molar / (M_alloy / 1000)`
                - All fitted coefficients are also provided in specific form

                **Numerical Stability:**
                - Sigmoid term uses `np.clip()` to prevent overflow
                - `max(T - Tₘ, 0)` ensures piecewise continuity
                - Bounds enforcement during fitting prevents unphysical parameters
                """)
        ...
        with col_h2:
            ...
            with col_data1:
                if st.button("🗑️ Clear All Data", type="secondary", width='stretch', key="tab6_clear_data"):
            ...
            with col_data2:
                if st.button("💾 Export Session", type="secondary", width='stretch', key="tab6_export_session"):
                    session_data = {
                        'results_history': [
                            {k: (v.tolist() if hasattr(v, 'tolist') else v) 
                             for k, v in res.items()} 
                            for res in analyzer.results_history
                        ],
                        'fitting_results': analyzer.fitting_results,
                        'simulation_cache': {
                            k: {kk: (vv.tolist() if hasattr(vv, 'tolist') else vv) 
                                for kk, vv in v.items()} 
                            for k, v in st.session_state.simulation_cache.items()
                        },
                        'customizations': analyzer.plot_customizations,
                        'timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            ...
            **Key Features:**
            ...
            - **N-Component Support**: Automatic mole/weight fraction conversion

            **Core Libraries:**
            - `pycalphad`, `scipy`, `xarray` for thermodynamic calculations
            - `matplotlib`, `plotly` for visualization
            - `streamlit` for interactive web interface

            **Session State Management:**
            - Results are cached to prevent re-computation
            - Widget keys are properly namespaced to avoid conflicts
            ...
            st.markdown(
                ...
                f'Session: {len(analyzer.results_history)} results, '
                f'{len(analyzer.fitting_results)} fits | '
