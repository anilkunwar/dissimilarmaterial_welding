...
# ====================== TAB 3: LOOKUP TABLES (WITH SESSION STATE) ======================
with tab_tables:
...
""".format(
        ...
        st.session_state.generated_content = {
            ...
            'enth_b_df': st.session_state.get('table_data_enth_b', pd.DataFrame()),
        }
        
        st.success(f"✅ Files generated for project `{project_name}`!")
        
        # === DOWNLOAD BUTTONS (using session state for persistence) ===
        col_dl1, col_dl2, col_dl3 = st.columns(3)
        
        with col_dl1:
            st.download_button(
                label="📄 Download case.sif",
                data=st.session_state.generated_content['sif_content'],
                file_name=st.session_state.generated_content['sif_filename'],
                mime="text/plain",
                key=uk("dl", "sif")
            )
            st.download_button(
                label="🔧 getDensity_A.F90",
                data=st.session_state.generated_content['dens_f90_a'],
                file_name=f"getDensity_{st.session_state.generated_content['mat_a_name']}.F90",
                mime="text/plain",
                key=uk("dl", "dens_a")
            )
            st.download_button(
                label="🔧 getDensity_B.F90",
                data=st.session_state.generated_content['dens_f90_b'],
                file_name=f"getDensity_{st.session_state.generated_content['mat_b_name']}.F90",
                mime="text/plain",
                key=uk("dl", "dens_b")
            )
        ...
        with col_dl2:
            st.download_button(
                ...
                data=st.session_state.generated_content['cond_f90_a'],
                file_name=f"getThermalConductivity_{st.session_state.generated_content['mat_a_name']}.F90",
                mime="text/plain",
                key=uk("dl", "cond_a")
            )
            st.download_button(
                label="🔧 getThermalConductivity_B.F90",
                data=st.session_state.generated_content['cond_f90_b'],
                file_name=f"getThermalConductivity_{st.session_state.generated_content['mat_b_name']}.F90",
                mime="text/plain",
                key=uk("dl", "cond_b")
            )
            st.download_button(
                label="🔧 HeatSource.F90",
                data=st.session_state.generated_content['heat_f90'],
        ...
        with col_dl3:
            st.download_button(
                ...
                data=st.session_state.generated_content['cte_f90_a'],
                file_name=f"getThermalExpansivity_{st.session_state.generated_content['mat_a_name']}.F90",
                mime="text/plain",
                key=uk("dl", "cte_a")
            )
            st.download_button(
                label="🔧 getThermalExpansivity_B.F90",
                data=st.session_state.generated_content['cte_f90_b'],
                file_name=f"getThermalExpansivity_{st.session_state.generated_content['mat_b_name']}.F90",
                mime="text/plain",
                key=uk("dl", "cte_b")
            )
            # Lookup tables - using ACTUAL edited DataFrames from session state
            for table_key, label_prefix in [
                ('visc_a_df', 'Viscosity – Material A'),
                ('visc_b_df', 'Viscosity – Material B'),
                ('enth_a_df', 'Specific Enthalpy – Material A'),
                ('enth_b_df', 'Specific Enthalpy – Material B')
            ]:
                ...
                st.download_button(
                    ...
                    file_name=fname,
                    mime="text/tab-separated-values",
                    key=uk("dl", f"table_{label_prefix.replace(' ', '_')}")
                )
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
            ...
            zip_file.writestr(
                f"getDensity_{st.session_state.generated_content['mat_b_name']}.F90",
                st.session_state.generated_content['dens_f90_b']
            )
            zip_file.writestr(
                f"getThermalConductivity_{st.session_state.generated_content['mat_a_name']}.F90",
                st.session_state.generated_content['cond_f90_a']
            )
            zip_file.writestr(
                f"getThermalConductivity_{st.session_state.generated_content['mat_b_name']}.F90",
                st.session_state.generated_content['cond_f90_b']
            )
            zip_file.writestr(
                f"getThermalExpansivity_{st.session_state.generated_content['mat_a_name']}.F90",
                st.session_state.generated_content['cte_f90_a']
            )
            zip_file.writestr(
                f"getThermalExpansivity_{st.session_state.generated_content['mat_b_name']}.F90",
                st.session_state.generated_content['cte_f90_b']
            )
            ...
            for table_key, prefix in [
                ...
                df = st.session_state.generated_content[table_key]
                if not df.empty:
                    mat = st.session_state.generated_content['mat_a_name'] if 'a' in table_key else st.session_state.generated_content['mat_b_name']
