...
# Updated default values: length=400, width=100, height=20 for Box 1
...
# Default for Box 2: placed on top of Box 1, sharing same X and Z extents
...
def add_box_to_matplotlib(ax, xmin, ymin, zmin, xmax, ymax, zmax, color='blue', alpha=0.3):
    """Draw a box in a matplotlib 3D axes."""
    vertices = np.array([
    ...
    # Define the 6 faces (each face is a list of 4 vertex indices)
    faces = [
        [0,1,2,3],  # bottom
        [4,5,6,7],  # top
        [0,1,5,4],  # front
    ...
    collection = Poly3DCollection([vertices[face] for face in faces],
                                  alpha=alpha, facecolor=color, edgecolor=color, linewidth=1)
    ax.add_collection3d(collection)

# ====================== PLOTLY GEOMETRY PREVIEW ======================
fig_plotly = go.Figure()

# Box 1 (front) – from (0,0,0) to (v2x, v2y, v2z)
add_box_edges(fig_plotly, 0, 0, 0, v2x, v2y, v2z, "blue", "Box_1 (front)")
...
add_box_edges(fig_plotly, bx2_xmin, bx2_ymin, bx2_zmin, bx2_xmax, bx2_ymax, bx2_zmax, "red", "Box_2 (back)")
...
fig_plotly.update_layout(
    title="Live 3D Geometry Preview (Plotly)",
    scene=dict(
...
# ====================== MATPLOTLIB GEOMETRY PREVIEW ======================
fig_matplotlib = plt.figure(figsize=(8, 6))
ax = fig_matplotlib.add_subplot(111, projection='3d')

# Add both boxes
add_box_to_matplotlib(ax, 0, 0, 0, v2x, v2y, v2z, color='blue', alpha=0.3)
add_box_to_matplotlib(ax, bx2_xmin, bx2_ymin, bx2_zmin, bx2_xmax, bx2_ymax, bx2_zmax, color='red', alpha=0.3)

# Set labels and limits
ax.set_xlabel('X (Length)')
ax.set_ylabel('Y (Width)')
ax.set_zlabel('Z (Height)')
ax.set_title('Matplotlib 3D View')

# Set equal aspect ratio (approximate)
max_range = max(v2x, v2y, v2z, bx2_xmax-bx2_xmin, bx2_ymax-bx2_ymin, bx2_zmax-bx2_zmin)
mid_x = (0 + max(v2x, bx2_xmax)) / 2
mid_y = (0 + max(v2y, bx2_ymax)) / 2
mid_z = (0 + max(v2z, bx2_zmax)) / 2
...
def sanitize_filename(text):
    ...
    # Replace spaces and special chars with underscores, keep alphanumerics, dots, underscores, hyphens
    return re.sub(r'[^\w.\-]', '_', text)

# Calculate Box 2 spans
box2_span_x = bx2_xmax - bx2_xmin
box2_span_y = bx2_ymax - bx2_ymin  
box2_span_z = bx2_zmax - bx2_zmin

# Format dimensions with 1 decimal place for readability
box1_info = f"Solid1front_L{v2x:.1f}_W{v2y:.1f}_H{v2z:.1f}"
box2_info = f"Solid2back_X{box2_span_x:.1f}_Y{box2_span_y:.1f}_Z{box2_span_z:.1f}"

# Build descriptive filename
base_name = "butt_weld_joint"
filename = sanitize_filename(f"{base_name}_{box1_info}_{box2_info}.py")

# Optional: Add tooltip showing the full dimension info on hover
filename_tooltip = (f"Box 1 (Solid_1front): Length {v2x} × Width {v2y} × Height {v2z} | "
                   f"Box 2 (Solid_2back): X-span {box2_span_x} × Y-span {box2_span_y} × Z-span {box2_span_z}")
...
st.download_button(
    ...
    file_name=filename,
    mime="text/plain",
    help=filename_tooltip  # Shows dimension info on hover
)

st.success("✅ Script is ready! Load it in SALOME (File → Load Script) to see the exact geometry + mesh with your new dimensions.")
st.info("The 3D previews above show live wireframes of the geometry. The real mesh is generated inside SALOME.")

# ====================== FILENAME PREVIEW ======================
with st.expander("📁 View Generated Filename Details"):
    st.write(f"**Generated filename:** `{filename}`")
    st.write(f"**Box 1 (Solid_1front):** Length {v2x} × Width {v2y} × Height {v2z}")
    st.write(f"**Box 2 (Solid_2back):** X-span {box2_span_x} × Y-span {box2_span_y} × Z-span {box2_span_z}")
    st.caption("Filename uses underscores instead of spaces/special characters for compatibility.")
